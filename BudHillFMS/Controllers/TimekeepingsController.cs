...
namespace BudHillFMS.Controllers
{
    public class TimekeepingsController : Controller
    {
        ...
        {
            var farmManagementSystemContext = _context.Timekeepings.Include(t => t.Employee).OrderByDescending(t => t.TimekeepingDate)
        ...
        public IActionResult ExportToExcel(DateTime? startDate, DateTime? endDate)
        {
            ...
            byte[] excelData = GenerateExcelData(timekeepings);

            // Đặt tên file và loại nội dung
            string startDateString = startDate?.ToString("yyyy-MM-dd");
            string endDateString = endDate?.ToString("yyyy-MM-dd");

            string fileName = $"chamcong_{startDateString}_{endDateString}.xlsx";
